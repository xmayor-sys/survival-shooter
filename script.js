...
const upgrades = {
    ...
    AGUA_BENDITA: { id: 'AGUA_BENDITA', name: "Agua Bendita (Zona)", evolution: {requires: 'IMAN', to: 'SANTUARIO'}, levels: [{ description: "Lanza viales que crean zonas de daño.", apply: (p) => { p.holyWaterLevel = 1; p.holyWaterTimer = 0; p.holyWaterRate = 300; p.holyWaterRadius = 60;} },{ description: "Lanza viales más a menudo.", apply: (p) => { p.holyWaterRate = 200; p.holyWaterLevel = 2; p.holyWaterRadius = Math.round(p.holyWaterRadius * 1.10);} },{ description: "Las zonas de daño son más grandes.", apply: (p) => { p.holyWaterLevel = 3; p.holyWaterRadius = Math.round(p.holyWaterRadius * 1.10);} }], getCurrentLevel: (p) => p.holyWaterLevel || 0, canApply: (p) => (p.holyWaterLevel || 0) < 3, apply: (p) => { const level = upgrades.AGUA_BENDITA.getCurrentLevel(p); if (upgrades.AGUA_BENDITA.canApply(p)) { upgrades.AGUA_BENDITA.levels[level].apply(p); }}},
...
class Player extends Entity {
    ...
    updateHolyWater() {
        if (++this.holyWaterTimer >= Math.max(1800, this.holyWaterRate)) {
